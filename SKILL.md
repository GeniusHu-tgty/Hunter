...
1. `hunter` legacy MCP server????????? JSON string??? 45 ????
2. `hunter_tools` reverse_lab_tools-style server????? `hunter_tools_mcp.py`?? wrapper????? dict?????? `reverse_lab_tools` ?????
